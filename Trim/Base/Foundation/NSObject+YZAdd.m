...
#import "NSObject+YZAdd.h"
#import "YZTirmMacro.h"
#import <objc/objc.h>
#import <objc/runtime.h>

YZSYNTH_DUMMY_CLASS(NSObject_YZAdd)


@implementation NSObject (YZAdd)
...
- (id)yz_performSelectorWithArgs:(SEL)sel, ...{
    INIT_INV(sel, nil);
    [inv invoke];
    return [NSObject getReturnFromInv:inv withSig:sig];
}

- (void)yz_performSelectorWithArgs:(SEL)sel afterDelay:(NSTimeInterval)delay, ...{
    INIT_INV(delay, );
    [inv retainArguments];
    [inv performSelector:@selector(invoke) withObject:nil afterDelay:delay];
}

- (id)yz_performSelectorWithArgsOnMainThread:(SEL)sel waitUntilDone:(BOOL)wait, ...{
    INIT_INV(wait, nil);
...
- (id)yz_performSelectorWithArgs:(SEL)sel onThread:(NSThread *)thr waitUntilDone:(BOOL)wait, ...{
    INIT_INV(wait, nil);
    if (!wait) [inv retainArguments];
...
- (void)yz_performSelectorWithArgsInBackground:(SEL)sel, ...{
    INIT_INV(sel, );
    [inv retainArguments];
...
#define case_size(_size_) \
    ...
    [inv setArgument:&arg atIndex:index]; \
            ...
            case_size(61) case_size(62) case_size(63) case_size(64)
                ...
                NSLog(@"YZKit performSelectorWithArgs unsupported type:%s (%lu bytes)",
...
- (void)yz_performSelector:(SEL)sel afterDelay:(NSTimeInterval)delay {
    [self performSelector:sel withObject:nil afterDelay:delay];
}

+ (BOOL)yz_swizzleInstanceMethod:(SEL)originalSel with:(SEL)newSel {
...
+ (BOOL)yz_swizzleClassMethod:(SEL)originalSel with:(SEL)newSel {
...
- (void)yz_setAssociateValue:(id)value withKey:(void *)key {
    objc_setAssociatedObject(self, key, value, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)yz_setAssociateWeakValue:(id)value withKey:(void *)key {
    objc_setAssociatedObject(self, key, value, OBJC_ASSOCIATION_ASSIGN);
}

- (void)yz_removeAssociatedValues {
    objc_removeAssociatedObjects(self);
}

- (id)yz_getAssociatedValueForKey:(void *)key {
...
- (id)yz_deepCopyWithArchiver:(Class)archiver unarchiver:(Class)unarchiver {
    id obj = nil;
    @try {
