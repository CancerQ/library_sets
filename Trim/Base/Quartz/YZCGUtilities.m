...
CGContextRef YZCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef YZCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform YZCGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]) {
...
CGAffineTransform YZCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    after[0] = [from yz_convertPoint:before[0] toViewOrWindow:to];
    after[1] = [from yz_convertPoint:before[1] toViewOrWindow:to];
    after[2] = [from yz_convertPoint:before[2] toViewOrWindow:to];
    
    return YZCGAffineTransformGetFromPoints(before, after);
}
