...
YZSYNTH_DUMMY_CLASS(UIApplication_YZAdd)

#define kNetworkIndicatorDelay (1/30.0)
@interface _YZUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _YZUIApplicationNetworkIndicatorInfo
@end


@implementation UIApplication (YZAdd)
...
- (BOOL)isPirated {
    ...
    if (![self _yz_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _yz_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_yz_fileExistInMainBundle:(NSString *)name {
...
YZSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _YZUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _YZUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_YZUIApplicationNetworkIndicatorInfo new];
