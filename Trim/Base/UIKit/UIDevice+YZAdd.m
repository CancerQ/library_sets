...
YZSYNTH_DUMMY_CLASS(UIDevice_YZAdd)


@implementation UIDevice (YZAdd)
...
- (BOOL)isJailbroken {
    ...
    NSString *path = [NSString stringWithFormat:@"/private/%@", [NSString yz_stringWithUUID]];
...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} yz_net_interface_counter;


static uint64_t yz_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t yz_net_counter_get_by_type(yz_net_interface_counter *counter, YZNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & YZNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & YZNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & YZNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & YZNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & YZNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & YZNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static yz_net_interface_counter yz_get_net_interface_counter() {
    ...
    yz_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = yz_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = yz_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)getNetworkTrafficBytes:(YZNetworkTrafficType)types {
    yz_net_interface_counter counter = yz_get_net_interface_counter();
    return yz_net_counter_get_by_type(&counter, types);
}
...
- (NSString *)machineModelName {
    ...
    dispatch_once(&one, ^{
        ...
        NSDictionary *dic = @{
            ...
            @"iPhone10,6" : @"iPhone X",
            @"iPhone10,7" : @"iPhone XR",
            @"iPhone10,8" : @"iPhone XS",
            @"iPhone10,9" : @"iPhone XSMax",
